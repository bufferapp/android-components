...
import kotlinx.android.synthetic.main.view_item_selection_message_sheet.view.*
...
object BottomSheetFactory {
    ...
    fun createTextItemSelectionBottomSheet(
        ...
        return createTextItemSelectionBottomSheet(context, textColor, background, titleResource,
                messageResource = null, adapter = adapter, listener = listener, footerView = footerView)
    }
    ...
    fun createTextItemSelectionBottomSheet(
        ...
        val message = messageResource?.let { context.getString(messageResource) }
                ?: kotlin.run { null }
        return createTextItemSelectionBottomSheet(context, textColor, background, title, message,
                adapter, listener, footerView)
    }
    ...
    fun createTextItemSelectionBottomSheet(
        ...
        return BottomSheetDialog(context, R.style.BottomSheetDialog).apply {
            val view = layoutInflater.inflate(R.layout.view_item_selection_message_sheet, null)
                    as LinearLayout
            view.layout_root.background = ContextCompat.getDrawable(context, background)
            title?.let {
